...
const getValidEnvironmentVariables = () => {
  config();
  ...
  const validEnv = cleanEnv(process.env, {
     ...
     */
    JWT_SECRET: str({
      default: btoa(randomBytes(256).toString("hex")),
    }),
 ...
 * @example
 * ```ts
 * console.log(env.DB_NAME); // "sample_mflix"
 * console.log(env.PORT); // 3000
 * ```
 */
export const env = getValidEnvironmentVariables();
